...
class Solution {
  ...
  public ArrayList<Node> getNextPos(Node pos, int[][] board) {
    ...
    // 현재 로봇이 세로로 놓여 있는 경우
    int[] ver = {-1, 1};
    if (pos.getPos1Y() == pos.getPos2Y()) {  // 왼쪽으로 회전하거나, 오른쪽으로 회전
      // 왼쪽 혹은 오른쪽 두칸이 모두 비어 있다면
      for (int i = 0; i < 2; i++) {
       if (board[pos.getPos1X()][pos.getPos2Y() + ver[i]] == 0 && board[pos.getPos2X()][pos.getPos2Y() + ver[i]] == 0) {
