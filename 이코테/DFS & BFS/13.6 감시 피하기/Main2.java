...
class Main2 {
  ...
  int n;  // 복도의 크기
  char[][] board = new char[6][6];  // 복도 정보 (N x N)
  ArrayList<Position> teachers = new ArrayList<>();  // 모든 선생님 위치 정보
  ArrayList<Position> spaces = new ArrayList<>();  // 모든 빈 공간 위치 정보
  ...
  public boolean watch(int x, int y, int direction) {
    // 왼쪽 방향으로 감시
    if (direction == 0) {
      while (y >= 0) {
