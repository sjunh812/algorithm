...
class Solution2 {
  ...
  public int[] solution(String[] words, String[] queries) {
    ...
    // 모든 단어들을 접미사 와일드카드 배열, 접두사 와일드카드 배열에 각각 삽입
    for (int i = 0; i < words.length; i++) {
      String word = words[i];
      arr.get(word.length()).add(word);  // 단어를 삽입
      word = (new StringBuffer(word)).reverse().toString();
      reversedArr.get(word.length()).add(word);  // 단어를 뒤집어서 삽
    }
    ...
    for (int i = 0; i < queries.length; i++) {
      String q = queries[i];
      int res = 0;
      if (q.charAt(0) != '?') {  // 접미사에 와일드카드가 붙은 경우
  ...
  public int lowerBound(ArrayList<String> arr, String target, int start, int end) {
   ...
   public int upperBound(ArrayList<String> arr, String target, int start, int end) {
    while (start < end) {
      int mid = (start + end) / 2;
      // arr[mid]가 target보다 사전순으로 같거나 앞에 있다면
      if (arr.get(mid).compareTo(target) <= 0) start = mid + 1;
      else end = mid;
    }
